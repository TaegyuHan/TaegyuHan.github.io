---
title: "[DevOps] 자주 사용하는 PromQL 쿼리 리스트"

tagline: "노드 익스포터, cAdvisor, Spring Boot Actuator 메트릭을 PromQL로 조회·알림하는 방법"

header:
  overlay_image: /assets/post/devops/2026-01-24-promql-practical/overlay.png
  overlay_filter: 0.5
...
```
{: data-code-box-caption="CPU 사용률 (Node Exporter)" }

### 메모리 사용률(%)
```text
(node_memory_MemTotal_bytes - node_memory_MemAvailable_bytes) / node_memory_MemTotal_bytes * 100
```
{: data-code-box-caption="메모리 사용률 (Node Exporter)" }
...
```
{: data-code-box-caption="디스크 사용률 (Node Exporter)" }
...
```
{: data-code-box-caption="네트워크 수신/송신 Bps (Node Exporter)" }
...
```
{: data-code-box-caption="컨테이너 CPU 사용률 (cAdvisor)" }

### 컨테이너 메모리 사용 바이트
```text
sum by (container) (container_memory_working_set_bytes{image!=""})
```
{: data-code-box-caption="컨테이너 메모리 사용량 (cAdvisor)" }

### 컨테이너 재시작 감지(최근 10분)
```text
increase(container_last_seen{image!=""}[10m]) < 1
```
{: data-code-box-caption="최근 10분 내 컨테이너 재시작 여부 (cAdvisor)" }
...
```
{: data-code-box-caption="컨테이너 네트워크 수신/송신 Bps (cAdvisor)" }

---

## 3. Spring Boot Actuator

{% include post/param-replacer.html key="job" default="spring-app" %}

### RPS
```text
sum by (instance) (rate(http_server_requests_seconds_count{job="spring-app"}[1m]))
```
{: data-code-box-caption="초당 요청 수 (Spring Actuator)" }
...
```
{: data-code-box-caption="p95 응답시간 (Spring Actuator)" }
...
```
{: data-code-box-caption="Spring 앱 에러율 비교" }

### JVM 힙 사용량
```text
jvm_memory_used_bytes{job="spring-app",area="heap"}
```
{: data-code-box-caption="JVM 힙 사용량 (Spring Actuator)" }

### GC Pause 합계(5m)
```text
sum by (gc) (rate(jvm_gc_pause_seconds_sum{job="spring-app"}[5m]))
```
{: data-code-box-caption="GC Pause 합계 5분 (Spring Actuator)" }